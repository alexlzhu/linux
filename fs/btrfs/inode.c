...
retry:
		...
		ret = btrfs_add_ordered_extent(inode,
					       async_extent->start,	/* file_offset */
					    ...
					    struct bio *bio, int mirror_num)
{
	...
	struct btrfs_io_bio *bbio = btrfs_io_bio(bio);
	struct btrfs_fs_info *fs_info = inode->root->fs_info;
	blk_status_t ret;
...
static blk_status_t btrfs_encoded_read_verify_csum(struct btrfs_io_bio *bbio)
{
					  ...
					  u64 disk_io_size, struct page **pages)
{
	...
	while (cur < disk_io_size) {
		...
		while (bio || remaining) {
			...
			if (!bio) {
				bio = btrfs_bio_alloc(disk_bytenr + cur);
