...
TRACE_EVENT(hostres,
	TP_PROTO(u32 lock_state, u32 is_user, u64 handle, int readers, u32 is_read),
	...
	NNP_TP_fast_assign(
		       __entry->lock_state = lock_state;
		       __entry->is_user = is_user;
		       __entry->handle  = handle;
		       __entry->readers = readers;
		       __entry->is_read = is_read;
	),
	NNP_TP_printk("lock_state=%s is_user=%d handle=0x%llx readers=%d is_read=%d",
...
/*TRACE_EVENT(NNP_TRACE_MMIO,
	TP_PROTO(char op, u32 offset, u64 val),
	...
	NNP_TP_printk("op=%c off=0x%x val=0x%llx",
		__entry->op,
		__entry->offset,
		__entry->val)
);
