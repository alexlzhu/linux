					...
					size_t                   count)
{
	...
	if ((count <= 1) ||
		...
		nnp_log_err(START_UP_LOG, "Invalid Input. Input should be: <type_str,clock> or <type_str>. size: %lu\n", count);
		return -EINVAL;
	}
	...
	for (i = 0; i < count; i++) {
		if (buf[i] == ',' || buf[i] == '\n')
			...
			nnp_log_err(START_UP_LOG, "Invalid clock. Input should be: <type_str,clock> or <type_str>. size: %lu\n", count);
			return -EINVAL;
		} else { /* SPH_IGNORE_STYLE_CHECK */
	...
	DO_TRACE(trace_nnpi_host_clock_stamp(clock_type_host, clock_msg.i_clock, nnpdev->id));
					   ...
					   const char **fail_reason)
{
	...
	else if (nnpdev->state &
		 (NNP_DEVICE_BOOT_BIOS_READY |
		  NNP_DEVICE_BOOT_RECOVERY_BIOS_READY |
		  NNP_DEVICE_BOOT_SYSINFO_READY))
		*boot_state = nnpdev->is_recovery_bios ? "Recovery Bios Ready" : "Bios Ready";
	else
		*boot_state = "Unknown";

	/* Find Failure Reason*/
	if (nnpdev->state & (NNP_DEVICE_FAILED_VERSION | NNP_DEVICE_PROTOCOL_ERROR))
...
static DEVICE_ATTR(channels, 0400, channels_show, NULL);
...
static DEVICE_ATTR(crashlog_size, 0400, crashlog_size_show, NULL);
...
static DEVICE_ATTR(ipc_event_h2c_en, 0644, ipc_event_h2c_en_show, ipc_event_h2c_en_store);
...
static DEVICE_ATTR(ipc_event_c2h_en, 0644, ipc_event_c2h_en_show, ipc_event_c2h_en_store);
				    ...
				    char                    *buf)
{
	...
	ret += snprintf(&buf[ret], PAGE_SIZE - ret, "dma_hang_nonfatal: %llu\n",
			nnpdev->counters.uncorr.dma_hang_nonfatal);
	ret += snprintf(&buf[ret], PAGE_SIZE - ret, "dma_hang_fatal: %llu\n",
...
int nnpdrv_device_sysfs_init(struct nnp_device *nnpdev)
{
	...
	ret = sysfs_chmod_file(&nnpdev->cdev->kobj, &dev_attr_ipc_event_h2c_en.attr, 0666);
	ret |= sysfs_chmod_file(&nnpdev->cdev->kobj, &dev_attr_ipc_event_h2c_dis.attr, 0666);
	ret |= sysfs_chmod_file(&nnpdev->cdev->kobj, &dev_attr_ipc_event_c2h_en.attr, 0666);
	ret |= sysfs_chmod_file(&nnpdev->cdev->kobj, &dev_attr_ipc_event_c2h_dis.attr, 0666);
	ret |= sysfs_chmod_file(&nnpdev->cdev->kobj, &dev_attr_clock_stamp.attr, 0666);
	if (ret)
		return ret;
