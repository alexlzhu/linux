...
#define NNP_IDR_ALLOC(p) nnp_idr_alloc(&proc_info->objects_idr, (p))
#define NNP_IDR_GET_OBJECT(id, fn_check_and_get) nnp_idr_get_object(&proc_info->objects_idr, (int)(id), (fn_check_and_get))
#define NNP_IDR_REMOVE_OBJECT(id) nnp_idr_remove_object(&proc_info->objects_idr, (int)(id))
#define NNP_IDR_CHECK_AND_REMOVE_OBJECT(id, fn_check) nnp_idr_check_and_remove_object(&proc_info->objects_idr, (int)(id), (fn_check))
			   ...
			   void __user             *arg)
{
	...
	if (create_args.size == 0) { //dma_buf fd is valid
		...
		if (likely(ret == 0))
			create_args.size = nnpdrv_hostres_get_size(hostres);
	} else if (create_args.user_ptr != 0) {
		ret = nnpdrv_hostres_create_usermem(create_args.user_ptr,
						    create_args.size,
						    convert_nnp2dma_direction(create_args.usage_flags),
						    &hostres);
		fd = -1;
			    ...
			    void __user             *arg)
{
	...
	inf_hostres_entry = NNP_IDR_CHECK_AND_REMOVE_OBJECT(destroy_args.user_handle,
							   is_inf_hostres_ptr);
...
finish:
	if (unlikely(copy_to_user(arg, &destroy_args, sizeof(destroy_args)) != 0))
			 ...
			 void __user             *arg)
{
	...
	ret = nnpdrv_hostres_user_lock(hostres, 0);
	if (unlikely(ret < 0)) {
		nnp_log_debug(GENERAL_LOG, "Failed to lock host resource 0x%x. err:%d\n", lock_args.user_handle, ret);
		goto finish;
	}
			   ...
			   void __user             *arg)
{
	...
	if (unlikely(ret < 0)) {
		nnp_log_err(GENERAL_LOG, "Failed to unlock hostres 0x%x. err:%d.\n", lock_args.user_handle, ret);
		goto finish;
	}
...
{
	long ret = 0;
	struct inf_process_info *proc_info = (struct inf_process_info *)f->private_data;
