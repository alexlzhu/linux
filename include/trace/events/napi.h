...
DECLARE_EVENT_CLASS(dql_stall_event,

	TP_PROTO(unsigned long thrs, unsigned long last_reap,
		 unsigned long hist_head, unsigned long now,
		 unsigned long *hist),

	TP_ARGS(thrs, last_reap, hist_head, now, hist),

	TP_STRUCT__entry(
		__field(	unsigned long,		thrs)
	...
	TP_printk("thrs %lu  last_reap %lu  hist_head %lu  now %lu  hist %016lx %016lx %016lx %016lx",
		  __entry->thrs, __entry->last_reap,
		  __entry->hist_head, __entry->now,
		  __entry->hist[0], __entry->hist[1],
		  __entry->hist[2], __entry->hist[3])
);

DEFINE_EVENT(dql_stall_event, dql_stall_detected,
